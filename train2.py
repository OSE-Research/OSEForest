...
FILENAME = 'train.csv'
TESTDIR = 'labeled_msad'
TESTPATH = 'msad.csv'
LABEL = 0
THRESHOLD = 0.9

'''train data '''
total = pd.read_csv('/home/dell/DNS/data/' + FILENAME, low_memory=False)
y = total['label']
...
data_test.to_csv('msad.csv', index=False, header=True)
...
np.savetxt('./result/msad_pred.txt', y_pred)
...
np.savetxt('./result/msad.txt', predictions)
